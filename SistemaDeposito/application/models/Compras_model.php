...
class Compras_model extends CI_Model
{
	...
	{
		$this->db->select("c.*,p.nit,p.nombre as proveedor, u.nombres");


		$this->db->select("c.*,al.precio,al.nombre as almacenes,");

		//$this->db->select("dc.*,pr.codigo_barras,pr.nombre,pr.stock");
		//$this->db->from("detalle_compra dc");
		//$this->db->join("productos pr", "dc.producto_id = pr.id");
	...
	{
		$this->db->select("v.*,al.nombre as almacenes,al.precio as costo");
		$this->db->join("almacen al", "v.almacen_id = al.id");

		$this->db->select("v.*,c.nombre as nombreproveedor,u.nombres");
		$this->db->from("compras v");
		$this->db->join("proveedor c", "v.proveedor_id = c.id");
	...
	public function getVentas()
	{
		...
		$this->db->select("v.*,al.nombre as almacenes,al.precio as costo");
		$this->db->join("almacen al", "v.almacen_id = al.id");
		$this->db->select("v.*,p.nombre as nombreproveedor,u.nombres");
		$this->db->from("compras v");
		$this->db->join("proveedor p", "v.proveedor_id = p.id");
		//$this->db->join("tipo_comprobante tc", "v.tipo_comprobante_id = tc.id");
		$this->db->join("usuarios u", "v.usuario_id = u.id");
